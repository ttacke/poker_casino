	...
	describe("Angenommen ich bin ein Poker-Croupier für Texas", function() {
		...
		function blatt(a) {
			...
			return {
				schlaegt: function(b) {
					...
					if(!c.blattAschlaegtB(blattA, blattB)) {
		...
		describe("UNITTESTS", function() {
			it("String in Karten konvertieren", function() {
				...
				expect(k[0].wert).toBe("K");
				expect(k[0].farbe).toBe("♥");
				expect(k[1].wert).toBe("10");
		...
		it("Blatt-Bewertung", function() {
			...
			blatt("HighCard Q♣ J♣").unterliegt("HighCard K♦ 10♦");
			...
			blatt("Flush ♠ Q♠ J♠ 9♠ 6♠").schlaegt("Straight 8♠ 7♠ 6♥ 5♥ 4♠");
			
			blatt("Flush A♥ Q♥ 10♥ 5♥ 3♥").schlaegt("Flush ♠ Q♠ J♠ 9♠ 6♠");
